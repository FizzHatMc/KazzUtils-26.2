...
/**
 * Custom render pipeline for in-world border boxes (wireframe boxes).
 *
 * Provides two rendering modes:
 * - **See-through**: Uses a custom pipeline without depth/stencil testing so the
 *   box is visible through walls.
 * - **Normal**: Uses the standard [RenderPipelines.DEBUG_FILLED_BOX] pipeline so the
 *   box is occluded by solid blocks.
 *
 * Each box is rendered as a wireframe (12 edges), where each edge is a thin
 * oriented rectangular prism connecting two adjacent corners.
 */
object BorderBoxRenderer {
    ...
    /** Shared buffer for all border box geometry. */
    ...
    /** Snapshot of border boxes to render this frame (set during extraction). */
    private var currentBorderBoxes: List<BorderBox> = emptyList()
     ...
     */
    fun initialize() {
        LevelExtractionEvents.END_EXTRACTION.register(this::extractBorderBoxes)
        LevelRenderEvents.AFTER_TRANSLUCENT_TERRAIN.register(this::renderAndDrawBorderBoxes)
    }

    // ── Extraction phase ─────────────────────────────────────────────────

    private fun extractBorderBoxes(context: LevelExtractionContext) {
        // Snapshot the current boxes (expired ones are pruned by the manager)
        currentBorderBoxes = BorderBoxManager.getActiveBorderBoxes()
    }

    // ── Drawing phase ────────────────────────────────────────────────────

    private fun renderAndDrawBorderBoxes(context: LevelRenderContext) {
        if (currentBorderBoxes.isEmpty()) return

        // Split into see-through and normal groups so we can batch by pipeline
        val seeThrough = currentBorderBoxes.filter { it.seeThrough }
        val normal = currentBorderBoxes.filter { !it.seeThrough }
    ...
    /**
     * Render a group of border boxes using the given [pipeline], then draw.
     */
    private fun renderGroup(context: LevelRenderContext, boxes: List<BorderBox>, pipeline: RenderPipeline) {
        ...
        for (box in boxes) {
            renderBorderBox(matrices, builder, box)
        }
    ...
    /**
     * Render a single border box (wireframe) into the vertex builder.
     */
    private fun renderBorderBox(matrices: PoseStack, builder: VertexConsumer, box: BorderBox) {
        val positionMatrix = matrices.last().pose()
        val r = box.color.red / 255f
        val g = box.color.green / 255f
        val b = box.color.blue / 255f
        val a = box.color.alpha / 255f

        // Compute the 8 corners of the axis-aligned box
        val half = box.scale * 0.5
        val cx = box.centerX
        val cy = box.centerY
        val cz = box.centerZ

        val corners = arrayOf(
            Vector3d(cx - half, cy - half, cz - half), // 0: bottom-left-back
            Vector3d(cx + half, cy - half, cz - half), // 1: bottom-right-back
            Vector3d(cx + half, cy + half, cz - half), // 2: top-right-back
            Vector3d(cx - half, cy + half, cz - half), // 3: top-left-back
            Vector3d(cx - half, cy - half, cz + half), // 4: bottom-left-front
            Vector3d(cx + half, cy - half, cz + half), // 5: bottom-right-front
            Vector3d(cx + half, cy + half, cz + half), // 6: top-right-front
            Vector3d(cx - half, cy + half, cz + half), // 7: top-left-front
        )
        ...
        for (edge in edges) {
            ...
            renderSolidPrism(positionMatrix, builder, edgeCorners, r, g, b, a)
        }
    }
    ...
    private fun computePrismCorners(
        ...
        if (length < 1e-10) {
            // Degenerate: zero-length edge, render a small box at the start point
            val h = thickness / 2.0
            return arrayOf(
    ...
    private fun drawToScreen(client: Minecraft, info: StagedVertexBuffer.ExecuteInfo, pipeline: RenderPipeline) {
        ...
        val renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
            { "kazzutils border box rendering" },
            colorTexture,
            Optional.empty(),
